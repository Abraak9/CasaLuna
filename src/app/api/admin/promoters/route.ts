...
export async function POST(req: NextRequest) {
  ...
  const { name, email, phone, commission_type, commission_value, promo_code, notes } = body;
  ...
  const promoter = await queryOne(
    `INSERT INTO promoters (id, name, email, phone, commission_type, commission_value, promo_code, notes)
     VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING *`,
    [uuidv4(), name, email || null, phone || null, commission_type || 'percentage', commission_value || 0, promo_code ? promo_code.toUpperCase() : null, notes || null]
  );
